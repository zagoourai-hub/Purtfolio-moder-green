...
import { usePost, useUpdatePost } from "@/hooks/use-blog";
  ...
  useEffect(() => {
    if (post) {
      setFormData({
        ...
        tagsRaw: Array.isArray(post.tags) ? post.tags.join(", ") : "",
  ...
  return (
    <div className="space-y-6 max-w-4xl">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-zinc-100">Edit Post</h1>
          <p className="text-sm text-zinc-400">Modify the article details and content.</p>
        </div>
        <Button
          variant="outline"
          size="sm"
      ...
      <Card className="border-zinc-800 bg-zinc-900/20 backdrop-blur-md">
        <CardHeader>
          ...
          <CardDescription className="text-zinc-400">
            Modify the fields below to update the blog post.
          </CardDescription>
        </CardHeader>
        <CardContent>
